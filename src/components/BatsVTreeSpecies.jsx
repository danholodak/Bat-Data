...
export default function BatsVTreeSpecies({ chartTitle }) {
  ...
  const updateChart = () => {
    ...
    const batFiles = uploadedFiles.filter(file => file.name.startsWith("bat"));
    const treeFiles = uploadedFiles.filter(file => file.name.startsWith("tree"));
    ...
    batFiles.forEach(file => {
      Papa.parse(file, {
        ...
        complete: function (sheet) {
          ...
          sheet.data.filter(recording => recording["PARK"]).forEach(recording => {
    ...
    treeFiles.forEach(file => {
      Papa.parse(file, {
        ...
        complete: function (sheet) {
          ...
          sheet.data.filter(treeMeasurement => treeMeasurement["PARK"]).forEach(treeMeasurement => {
